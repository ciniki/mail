...
function ciniki_mail_mailingImageAdd(&$ciniki) {
    ...
    $rc = ciniki_mail_checkAccess($ciniki, $args['business_id'], 'ciniki.mail.mailingImageAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
	...
	if( $rc['stat'] != 'ok' ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2136', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err']));
	}
	$args['uuid'] = $rc['uuid'];
	...
	if( $rc['num_rows'] > 0 ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2137', 'msg'=>'You already have an image with this name, please choose another name'));
	}


	if( $args['mailing_id'] <= 0 ) {
		return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2138', 'msg'=>'No mailing specified'));
	}
	...
	return ciniki_core_objectAdd($ciniki, $args['business_id'], 'ciniki.mail.mailing_image', $args, 0x07);
}
?>
